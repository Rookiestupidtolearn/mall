...
//		treeMap.put("owner", 			owner);
     ...
     * @return
     */
	public Map<String, Object> yeepayRechargeSubmmit(NideshopUser nideshopUser, ThirdPreCompanyRechargeRecord oldThirdPreCompanyRechargeRecord, String ip) {
		Map<String, Object> resultObj = new HashMap<String, Object>();
		//创建易宝订单实体
		YeeTradeOrderEntity entity = new YeeTradeOrderEntity();
		...
		entity.setUserId(Integer.valueOf(nideshopUser.getId()+"") );
		entity.setPayStatus(0); //初始
		entity.setPayType(0);
...
////		treeMap.put("fcallbackurl", 	fcallbackurl);
		//需要配置回调地址
		treeMap.put("callbackurl", 		PaymobileUtils.getRequestUrl(PaymobileUtils.AppIdRechargeCallbackApi_NAME));
//		treeMap.put("paytypes", 		paytypes);
...
//		treeMap.put("owner", 			owner);
	 ...
	 * @return
	 */
	public Map<String, Object> yeepayMemberRechargeSubmmit(QzRechargeRecordEntity qzRechargeRecordEntity, String ip) {
		Map<String, Object> resultObj = new HashMap<String, Object>();
		//创建易宝订单实体
		YeeTradeOrderEntity entity = new YeeTradeOrderEntity();
		entity.setYeeOrderNo(GenerateCodeUtil.buildBizNo("YB"));
		entity.setTradeNo(qzRechargeRecordEntity.getTradeNo());
		entity.setUserId(qzRechargeRecordEntity.getShopUserId());
		entity.setPayStatus(0); //初始
		entity.setPayType(0);
		entity.setAmount(qzRechargeRecordEntity.getAmount());
		//orderexpdate, currency, version是非必传参数
//		if(request.getParameter("orderexpdate") == null) {
//			//如果该参数不填写，设置为24小时
//			orderexpdate	= 24*60;
...
//			version		= ConvertUtils.objectToInt(request.getParameter("version"));
//		}
		int amount = qzRechargeRecordEntity.getAmount().multiply(new BigDecimal(100)).intValue();//实际下单的金额
		//使用TreeMap
		TreeMap<String, Object> treeMap	= new TreeMap<String, Object>();
		treeMap.put("orderid", 			entity.getYeeOrderNo());
		treeMap.put("transtime", 		Calendar.getInstance().getTimeInMillis() / 1000 );//带秒的时间戳
...
//		treeMap.put("userip", 			"192.168.1.1");
		...
		treeMap.put("fcallbackurl", 	PaymobileUtils.getRequestUrl(PaymobileUtils.ORDER_FCALL_BACKAPI_NAME));
		treeMap.put("callbackurl", 		PaymobileUtils.getRequestUrl(PaymobileUtils.MEMBERRECHARGE_CALLBACKAPI_NAME));
//		treeMap.put("paytypes", 		paytypes);
